...
<!-- STATS:START (auto-generated by tools/build_readme_stats.py — do not edit by hand) -->

**181 datasets · 6,307 entries · 38 categories** — authoritative sources mapped in [`meta/sources.json`](/meta/sources.json)

<details><summary>Per-category breakdown</summary>

| Category | Datasets | Entries |
|---|--:|--:|
| accessibility | 2 | 93 |
| ai | 1 | 32 |
| astronomy | 3 | 173 |
...
62 HTTP response codes with names, descriptions, categories (1xx–5xx), cacheability, and RFC sources.  
...
All 24 BIPM SI prefixes from quetta (10³⁰) to quecto (10⁻³⁰), including the four added by the 27th CGPM in 2022. Each entry gives the name, symbol, power-of-ten exponent, exact decimal multiplier, and year introduced.

[Info & Schema](/science/si-prefixes/Readme.MD) · [si-prefixes.json](/science/si-prefixes/si-prefixes.json)

### SI Base Units

The seven SI base units (second, metre, kilogram, ampere, kelvin, mole, candela), each with the fundamental constant that defines it under the 2019 SI redefinition.

[Info & Schema](/science/si-base-units/Readme.MD) · [si-base-units.json](/science/si-base-units/si-base-units.json)

### SI Derived Units

The 22 SI derived units with special names (hertz, newton, pascal, joule, watt, … katal), each expressed in other SI units and in SI base units.

[Info & Schema](/science/si-derived-units/Readme.MD) · [si-derived-units.json](/science/si-derived-units/si-derived-units.json)

### Binary Prefixes

The 8 IEC 80000-13 binary prefixes (kibi…yobi) for powers of two, with exact values and the SI decimal prefix each is commonly confused with — the KB-vs-KiB reference.

[Info & Schema](/science/binary-prefixes/Readme.MD) · [binary-prefixes.json](/science/binary-prefixes/binary-prefixes.json)
...
14 famous mathematical constants (π, e, φ, τ, √2, Euler–Mascheroni, Apéry's, Catalan's, …) to 30 significant figures, computed with arbitrary-precision arithmetic. Each has an exact-as-stored value string, a double approximation, a definition, and its OEIS identifier.

[Info & Schema](/math/constants/Readme.MD) · [constants.json](/math/constants/constants.json)
...
### Special-Use IP Blocks

49 IANA-registered special-purpose IPv4 and IPv6 address blocks — private-use, loopback, link-local, CGN shared space, and documentation/test ranges — each with its defining RFC(s) and IANA routing properties (forwardable, globally reachable, reserved-by-protocol).

[Info & Schema](/networking/special-use-ips/Readme.MD) · [special-use-ips.json](/networking/special-use-ips/special-use-ips.json)
...
The complete ISO 639-2 set of 487 three-letter language codes (Library of Congress), with bibliographic and terminologic variants, the ISO 639-1 two-letter code where one exists, and English + French names. Cross-consistent with the ISO 639-1 languages dataset.

[Info & Schema](/iso/639-2/Readme.MD) · [languages.json](/iso/639-2/languages.json)
...
All 64 mRNA codons of the standard genetic code (NCBI translation table 1), each mapped to its amino acid (cross-referencing the amino-acids dataset) or stop signal, with the canonical AUG start codon flagged.

[Info & Schema](/bio/codon-table/Readme.MD) · [codon-table.json](/bio/codon-table/codon-table.json)
...
21 cryptographic (and reference non-cryptographic) hash functions — MD family, SHA-0/1, SHA-2, SHA-3/SHAKE, BLAKE2/BLAKE3, RIPEMD-160, Whirlpool — with digest size, construction, year, and current security status (secure / broken).

[Info & Schema](/security/hash-algorithms/Readme.MD) · [hash-algorithms.json](/security/hash-algorithms/hash-algorithms.json)
